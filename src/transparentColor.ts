...
export const setTransparentPalette = ({
	...
	SCALE_LEVELS.forEach((level) => {
		...
		if (!normalizedColor || normalizedColor === '' || normalizedColor === 'undefined') {
			console.warn(`Invalid target solid color for level ${level}: "${targetSolidColor}"`);
			return;
		}
		...
		const backgroundColor =
			level <= transparentOriginLevel ? colorConfig.bgColorLight : colorConfig.bgColorDark;
...
const calculateTransparentColor = ({
	...
	try {
		...
		if (isNaN(target.r) || isNaN(target.g) || isNaN(target.b)) {
			console.error(`Invalid RGB from target color "${targetSolidColor}":`, target);
			return `rgba(0, 0, 0, ${fixedAlpha.toFixed(3)})`;
		}
	} catch (error) {
		console.error(`Error converting target color "${targetSolidColor}":`, error);
		return `rgba(0, 0, 0, ${fixedAlpha.toFixed(3)})`;
	}
	...
	try {
		...
		if (isNaN(bg.r) || isNaN(bg.g) || isNaN(bg.b)) {
			console.error(`Invalid RGB from background color "${backgroundColor}":`, bg);
			return `rgba(0, 0, 0, ${fixedAlpha.toFixed(3)})`;
		}
	} catch (error) {
		console.error(`Error converting background color "${backgroundColor}":`, error);
		return `rgba(0, 0, 0, ${fixedAlpha.toFixed(3)})`;
	}

	// ゼロ除算の防止
	if (fixedAlpha === 0) {
		console.warn('Alpha is 0, returning background color');
