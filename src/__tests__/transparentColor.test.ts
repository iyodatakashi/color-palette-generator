  ...
  describe("setTransparentPalette", () => {
    ...
    it("transparentOriginLevelで最大透過度が設定される", () => {
      ...
      testCases.forEach(({ originLevel, testLevel }) => {
        const config = {
          ...baseColorConfig,
          transparentOriginLevel: originLevel,
        };
        const palette = { ...basePalette };
      ...
      Object.keys(palette)
        ...
        .forEach((key) => {
          ...
          if (match) {
            ...
            // 精度が3桁以下であることを確認
            const decimalPlaces = alphaStr.includes(".")
              ? alphaStr.split(".")[1].length
              : 0;
    ...
    it("スケールレベルの境界値で正しく動作する", () => {
      ...
      const matchMin = alphaAtMin.match(
        /rgba\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*([\d.]+)\s*\)/
      );
      if (matchMin) {
      ...
      const matchMax = alphaAtMax.match(
        /rgba\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*([\d.]+)\s*\)/
      );
      if (matchMax) {
